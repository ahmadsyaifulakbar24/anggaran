...
function get_by_province(province_id) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $("#total-province").html(
                `Total Komponen: ${convert(value.total_work_plan)}`
            );
            $("#total-province-budged").html(
                `Total Anggaran ACC: ${rupiah(value.total_budged)}`
            );
        },
    });
    $.ajax({
            ...
            if (result.data != 0) {
                $.each(result.data, function (index, value) {
                    ...
                    append = `<tr>
	            		...
	            		<td class="text-truncate">${convert(
                            value.work_plan.total_target
                        )} ${value.work_plan.unit_target.name}</td>
...
function get_by_indicator(indicator_id) {
    ...
    $.ajax({
        ...
        success: function (result) {
            // console.log(result.data)
            let value = result.data;
            $("#total-indicator").html(
                `Total Komponen: ${convert(value.total_work_plan)}`
            );
            $("#total-indicator-budged").html(
                `Total Anggaran ACC: ${rupiah(value.total_budged)}`
            );
        },
    });
            ...
            if (result.data != 0) {
                $.each(result.data, function (index, value) {
                    ...
                    append = `<tr>
	            		...
	            		<td class="text-truncate">${convert(value.total_target)} ${
                        value.unit_target.name
                    }</td>
	            		<td class="text-truncate">${sub_work_plan}</td>
	            	</tr>`;
...
function get_by_assignment(assignment_id) {
    ...
    $.ajax({
        ...
        success: function (result) {
            // console.log(result.data)
            let value = result.data;
            $("#total-assignment").html(
                `Total Komponen: ${convert(value.total_work_plan)}`
            );
            $("#total-assignment-budged").html(
                `Total Anggaran ACC: ${rupiah(value.total_budged)}`
            );
        },
    });
    ...
    success: function (result) {
        ...
        $.each(result.data, function (index, value) {
            append = `<tr>
        		...
        		<td><b>${rupiah(value.budged_acc)}</b></td>
        		<td><b>${rupiah(value.budged_pending)}</b></td>
        	</tr>`;
